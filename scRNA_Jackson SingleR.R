...
### Find DEGs in SLE Vs. HC ---------------------------------------------------------

## Sample information and each cell type 
pbmc_ActiveIdent <- as.data.frame(pbmc@active.ident)

pbmc_ActiveIdent$Ident <- rownames(pbmc_ActiveIdent)

head(pbmc_ActiveIdent)
table(pbmc_ActiveIdent$`pbmc@active.ident`) ## 10 cell types

## read samples group
table(colsplit(pbmc_ActiveIdent$Ident,"-",c("A","ID"))$ID)  ## 13 sample serum

sample_group <- as.data.frame(read.csv2("aggregation.csv", sep = ","))
sample_group$ID <- seq(1,13)
sample_group <- sample_group[,-2]
head(sample_group)

pbmc_ActiveIdent$ID <- colsplit(pbmc_ActiveIdent$Ident,"-",c("A","ID"))$ID ## 13 sample serum
head(pbmc_ActiveIdent)

pbmc_ActiveIdent <- left_join(pbmc_ActiveIdent, sample_group)
head(pbmc_ActiveIdent)

table(pbmc_ActiveIdent$`pbmc@active.ident`)
# T cells, CD4+    T cells, CD8+          B cells         NK cells Monocytes, CD14+ Monocytes, CD16+               DC        Platelets 
# 34983             8495             3542            15732            16494             2764              514              224 

table(pbmc_ActiveIdent$group)
## 41632 aHD Vs. 41116 aSLE

table(pbmc_ActiveIdent[pbmc_ActiveIdent$group == "aHD",3]) ## 5
table(pbmc_ActiveIdent[pbmc_ActiveIdent$group == "aSLE",3]) ## 5

write.csv2(pbmc_ActiveIdent, "Final/scRNA_Jackson_meta_0812.cvs", quote = FALSE, row.names= FALSE)

## comparing cell type in aHD and aSLE
str(pbmc_ActiveIdent)
pbmc_ActiveIdent_1 <- pbmc_ActiveIdent
colnames(pbmc_ActiveIdent_1)[1] <- "CellType"


table(pbmc_ActiveIdent_1$group)

ggplot(pbmc_ActiveIdent_1, aes(x = group, y = CellType, fill = as.factor(CellType))) + 
  geom_bar(stat = "identity") + theme_classic2() + xlab("") + theme(legend.position = "NA") + ylab("Cell Count")

# ## Gene Expression Matrix (filter & scale)
 #GEM <- GetAssayData(object = pbmc, assay="RNA" ,slot = "count")
 GEM <- GetAssayData(object = pbmc, assay="RNA")
# GEM <- as.matrix(GEM)
# str(GEM)
# ## 23257 genes across 88606 cells
# GEM[1:5,1:5]
# 
# write.csv2(GEM, "scRNA_Jackson_GeneMatrix_Count.cvs", quote = FALSE)
...
# AB_genes <- c("SPARCL1","PARK7","VSIG4","C1QTNF1",
#               "TIMD4","CTSZ","CPA1","MCAM")
...
VlnPlot(pbmc, features = AB_genes, split.by = "Group", pt.size = 1) 
VlnPlot(pbmc, features = AB_genes, pt.size = 1) 

DotPlot(pbmc, features = AB_genes, cols = c("blue","red"), split.by = "Group") 

DimPlot(pbmc, reduction = "umap" , label = TRUE, pt.size = 0.5, split.by = "Group")

FeaturePlot(pbmc, reduction = "tsne", features = AB_genes, ncol = 5 ,pt.size = 0.5) & NoLegend() & NoAxes()

DoHeatmap(subset(pbmc, downsample = 1000), features = AB_genes)


### scRNA expression of VSIG4 -----------------------------------------------------------------

head(GEM)
dim(GEM) ## Genes * Cells
GEM[1:5,1:5]

GEM_VSIG4 <- GEM[which(rownames(GEM) %in% AB_genes),]

GEM_VSIG4 <- as.data.frame(GEM_VSIG4)
GEM_VSIG4$Gene <- rownames(GEM_VSIG4)

GEM_VSIG4_M <- melt(GEM_VSIG4)
head(GEM_VSIG4_M)
dim(GEM_VSIG4_M) ## 82748


## Check the SLE Vs. HD expression level in cell type of each 10 Biomarkers

head(GEM_VSIG4_M)
GEM_VSIG4_M$variable <- GEM_VSIG4_M$Gene
GEM_VSIG4_M$Gene <- "VSIG4"

head(pbmc_ActiveIdent)
table(pbmc_ActiveIdent$`pbmc@active.ident`)

pbmc_ActiveIdent_GEM_Raw <- left_join(GEM_VSIG4_M, pbmc_ActiveIdent, by = c("variable" = "Ident"))
colnames(pbmc_ActiveIdent_GEM_Raw) <- c("Gene" ,"Cells","ExpressLevel","CellType","SampleID","Sample_id","Group")

head(pbmc_ActiveIdent_GEM_Raw)
dim(pbmc_ActiveIdent_GEM_Raw)

write.csv(pbmc_ActiveIdent_GEM_Raw,"VSIG4_scRNA_serum_expression.csv")
...
write.csv2(IC_CellType, "Final/VSIG4_CellType_ExpressionComparing.csv")
